    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if (x == 0) or (x == DIGIT_DATUM_WIDTH - 1) or (y == 0) or (y == DIGIT_DATUM_HEIGHT - 1):
                gx[(x,y)] = datum.getPixel(x, y)
                gy[(x,y)] = datum.getPixel(x, y)
            else:
                gx[(x,y)] = datum.getPixel(x + 1, y) - datum.getPixel(x - 1, y)
                gy[(x,y)] = datum.getPixel(x, y + 1) - datum.getPixel(x, y - 1)

    mag = util.Counter()
    angle = util.Counter()
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            mag[(x,y)] = (gx[(x,y)] ** 2 + gy[(x,y)] ** 2) ** 0.5
            angle[(x,y)] = math.atan(gy[(x,y)] / (gx[(x,y)] + 0.1))
            features[(x,y,"m")] = mag[(x,y)]
            features[(x,y,"a")] = angle[(x,y)]
...
def enhancedPacmanFeatures(state, action):
    ...
    dist = sys.maxint
    num = 0
    pacmanState = state.generatePacmanSuccessor(action)

    for idx, ghost in enumerate(state.getGhostStates()):
        num = num + 1
        features[("g",idx)] = ghost.scaredTimer
        features[("d",idx)] = util.manhattanDistance(pacmanState.getPacmanPosition(), ghost.getPosition())

    for (x,y) in pacmanState.getCapsules():
        if util.manhattanDistance((x,y), pacmanState.getPacmanPosition()) < dist:
            dist = util.manhattanDistance((x,y), pacmanState.getPacmanPosition())
    if dist != sys.maxint:
        features['closest'] = dist / 2.
    else:
        features['closest'] = 0
    
    dist = sys.maxint
    for x in range(pacmanState.getFood().width):
        for y in range(pacmanState.getFood().height):
            if pacmanState.getFood()[x][y]:
                if util.manhattanDistance((x,y), pacmanState.getPacmanPosition()) < dist:
                    dist = util.manhattanDistance((x,y), pacmanState.getPacmanPosition())
    if dist != sys.maxint:
        features['closestF'] = dist
    else:
        features['closestF'] = 0
    
    features['score'] = pacmanState.getScore()
    return features
