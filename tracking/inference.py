...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for p in legal:
            for i in range(self.numParticles/len(legal)):
                self.particles.append([p, 1.0])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            self.particles = [[self.getJailPosition(), 1.0] for i in range(self.numParticles)]
        else:
            for idx, particle in enumerate(self.particles):
                self.particles[idx][1] = emissionModel[util.manhattanDistance(particle[0], pacmanPosition)]*particle[1]
            ...
            if beliefs.totalCount() == 0:
                ...
                for i in range(self.numParticles):
                    self.particles.append([util.sample(beliefs), 1.0])
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            temp.append([util.sample(newPosDist), 1.0])
        self.particles = temp
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            beliefs[particle[0]] = beliefs[particle[0]] + particle[1]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        allPossible = itertools.product(self.legalPositions, repeat=self.numGhosts)
        self.particles = []
        for p in allPossible:
            for i in range(self.numParticles/(len(self.legalPositions) ** self.numGhosts)):
                self.particles.append([p, 1.0])
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        eaten = []
        for idx, dist in enumerate(noisyDistances):
            if dist is None:
                eaten.append(idx)
                for idx2, particle in enumerate(self.particles):
                    temp = self.getParticleWithGhostInJail(self.particles[idx2][0], idx)
                    self.particles[idx2][0] = temp

        for idx, particle in enumerate(self.particles):
            temp = 1.0
            for i in range(self.numGhosts):
                if i not in eaten:
                    temp = emissionModels[i][util.manhattanDistance(particle[0][i], pacmanPosition)]*temp
            self.particles[idx][1] = temp*self.particles[idx][1]
        beliefs = self.getBeliefDistribution()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            temp = []
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle[0])), i, self.ghostAgents[i])
                temp.append(util.sample(newPosDist))
            newParticle = [tuple(temp), 1.0]
            "*** END YOUR CODE HERE ***"
